...
{{template.config.singleLineComment}} #end-scf-region`
    fsUtil.readFileContentSync.mockImplementation(path => {
    ...
    // ASSERT
    expect(fsUtil.saveFileSync.mock.calls.length).toBe(1)
    expect(fsUtil.saveFileSync.mock.calls[0][0]).toBe('/target/path/test.txt')
    expect(fsUtil.saveFileSync.mock.calls[0][1]).toBe(`# @scf-region
...
# @end-scf-region`)
  ...
  test('Render parameters in th folder and file paths', async () => {
    ...
    fsUtil.readFileContentSync.mockReturnValue('{{parameters.test}}')
    const ctx = {
      partialPath: '/source/path',
      partial: {
        name: '_init',
    ...
    // ASSERT
    expect(fsUtil.saveFileSync.mock.calls.length).toBe(1)
    expect(fsUtil.saveFileSync.mock.calls[0][0]).toBe('/target/path/folder-name/file-name.txt')
    expect(fsUtil.saveFileSync.mock.calls[0][1]).toBe('a sample test')
  })
